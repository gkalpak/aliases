...
module.exports = {
  /**
   * A configuration object, specifying the behavior of {@link index#run run()}.
   ...
   * if necessary). Also, derive a {@link index#RunConfig configuration object} (based on `--al-` arguments) to modify
   * the behavior of {@link index#run run()} (e.g. enable debug output).
   *
   ...
   *     substituting and a {@link index#RunConfig configuration object}.
   */
   ...
   *
   * You can use {@link index#preprocessArgs preprocessArgs()} to obtain the basic `runtimeArgs` and `config` values.
   ...
   * @param {RunConfig} [config={}] - A configuration object. See {@link index#RunConfig} for more details.
   *
...
function _run(cmd, runtimeArgs, config) {
  return doRun(cmd, runtimeArgs || [], config || {}).catch(onError);
}
...
function helpForCategory(catName, catSpec, joiner) {
  ...
  return [
    ...
    '',
    Object.keys(catSpec).
      map(name => ({name, spec: catSpec[name]})).
