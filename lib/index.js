  ...
  let expandedCmd = cmd.replace(re, (_, g1, g2, g3, g4, g5) => {
    ...
    if (!value && g5) {
      ...
      if (!match) {
        ...
        cmdPromises[cmd] = (cmdPromises[cmd] || doRun(cmd, runConfig)).then(result => {
          expandedCmd = expandedCmd.replace(placeholder, result.trim());
          return result;
        });
...
function insertAfter(newItem, afterItem, arr) {
  let idx = 0;
  while ((idx = arr.indexOf(afterItem, idx)) !== -1) arr.splice(++idx, 0, newItem);
}
