...
const {Alias, AliasDefault, AliasSpec, AliasSpecDefault} = require('./alias');
...
const G_PICKER_ALIAS = (things, pickerScriptName) => {
  ...
  const code = utils.stripIndentation(`
    ...
    if (require.main === module) {
      const {commandUtils} = require('@gkalpak/cli-utils');
      const config = commandUtils.preprocessArgs(process.argv.slice(2)).config;
...
const ALIASES = {
  ...
  git: {
    ...
    gl1g: new AliasDefault('(git log --decorate --oneline | grep $*) || true'),
    ...
    gbc: new AliasDefault('git checkout master -b $1 && git push --verbose --set-upstream origin $1'),
    ...
    gcl: new AliasDefault('git clean --interactive'),
    ...
    // BACKPORT
    gcp: new AliasDefault(utils.stripIndentation(`
    ...
    `).replace(/\r?\n/g, ' ')),
    gcpl: new AliasDefault('gcp ${0:::git rev-parse HEAD} ${1:::__g-pick-branch --gkcu-returnOutput=1}'),


    // PRIVATE
    '__g-pick-branch': G_PICKER_ALIAS('branches', 'g-pick-branch'),
    '__g-pick-commit': G_PICKER_ALIAS('commits', 'g-pick-commit'),
  },
  ...
  node: {
    ...
    nvls: new Alias({
      default: new AliasSpecDefault('. $NVM_DIR/nvm.sh && nvm list $*'),
      win32: new AliasSpecDefault('nvm list $*'),
    }),
    nvlsa: new Alias({
      default: new AliasSpecDefault('. $NVM_DIR/nvm.sh && nvm list-remote $*'),
      win32: new AliasSpecDefault('nvm list available $*'),
    }),
    nvu: new Alias({
      default: new AliasSpec(
        NVU_CODE,
        '. $NVM_DIR/nvm.sh && nvm use $* (it does not affect the currect process)'),
      win32: new AliasSpec(
        NVU_CODE,
    ...
    yap: new AliasDefault('yarn add $* --prod'),
    yad: new AliasDefault('yarn add $* --dev'),
    yrp: new AliasDefault('yarn remove $* --prod'),
    yrd: new AliasDefault('yarn remove $* --dev'),


    // RUN / TEST
    nr: new AliasDefault('npm run $*', {suppressTbj: true}),
    ns: new AliasDefault('npm start $*', {suppressTbj: true}),
    nt: new AliasDefault('npm test $*', {suppressTbj: true}),
    yr: new AliasDefault('yarn run $*', {suppressTbj: true}),
    ys: new AliasDefault('yarn start $*', {suppressTbj: true}),
    yt: new AliasDefault('yarn test $*', {suppressTbj: true}),
  ...
  aio: {
    ...
    // PRIVATE
    '__a-builds-dir': A_BUILDS_DIR_ALIAS(),
  },
