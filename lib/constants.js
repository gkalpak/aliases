...
const DEF_CODE = (cmd, cfg = {}) => utils.stripIndentation(`
  #!/usr/bin/env node
  'use strict';
...
const ALIASES = {
  ...
  git: {
    ...
    gl1g: '(git log --decorate --oneline | grep $*) || true',
    gsh: 'git show ${*:::__g-pick-commit --gkcu-returnOutput=1}',
    gshn: 'git show --name-only ${*:::__g-pick-commit --gkcu-returnOutput=1}',
    gd: 'ngm-diff-wh $*',
    gdn: 'ngm-diff-wh --name-only $*',
    gdh: 'ngm-diff-wh HEAD $*',
    gdnh: 'ngm-diff-wh HEAD --name-only $*',
    gd1: 'ngm-diff-wh HEAD~1 $*',
    gdn1: 'ngm-diff-wh HEAD~1 --name-only $*',
    ...
    // CHECKOUT
    gco: 'git checkout ${*:::__g-pick-branch --gkcu-returnOutput=1}',
    ...
    gcmfc: 'git commit --all --fixup ${0:::__g-pick-commit --gkcu-returnOutput=1} $*',
    gcmifc: 'git commit --fixup ${0:::__g-pick-commit --gkcu-returnOutput=1} $*',
    ...
    gbd: 'git branch --delete --force ${*:::__g-pick-branch --gkcu-returnOutput=1}',
    ...
    // REBASE
    grb: 'git rebase ${*:::__g-pick-branch --gkcu-returnOutput=1}',
    ...
    gcpl: 'gcp ${0:::git rev-parse HEAD} ${1:::__g-pick-branch --gkcu-returnOutput=1}',
  ...
  node: {
    ...
    // INSTALL / UNINSTALL
    nap: 'npm install $* --save',
    nad: 'npm install $* --save-dev',
    nrp: 'npm remove $* --save',
    nrd: 'npm remove $* --save-dev',
    yap: 'yarn add $* --prod',
    yad: 'yarn add $* --dev',
    yrp: 'yarn remove $* --prod',
    yrd: 'yarn remove $* --dev',


    // RUN / TEST
    nr: {cmd: 'npm run $*', cfg: {suppressTbj: true}},
    ns: {cmd: 'npm start $*', cfg: {suppressTbj: true}},
    nt: {cmd: 'npm test $*', cfg: {suppressTbj: true}},
    yr: {cmd: 'yarn run $*', cfg: {suppressTbj: true}},
    ys: {cmd: 'yarn start $*', cfg: {suppressTbj: true}},
    yt: {cmd: 'yarn test $*', cfg: {suppressTbj: true}},
