...
const ALIASES = {
  ...
  node: {
    ...
    srv: new AliasDefault(
      'light-server --bind="localhost" --historyindex="/${1:.}/index.html" --no-reload --serve="${1:.}" $2*',
      {suppressTbj: true}),
    srvw: new AliasDefault(
      'light-server --bind="localhost" --historyindex="/${1:.}/index.html" --serve="${1:.}" --watchexp="${1:.}/**" $2*',
      {suppressTbj: true}),
    naga: new AliasDefault(`npm install --global ${globalNpmPackages.join(' ')} $*`),
    yaga: new AliasDefault(`yarn global add ${globalNpmPackages.filter(x => !/^yarn(?:@|$)/.test(x)).join(' ')} $*`),
  },
