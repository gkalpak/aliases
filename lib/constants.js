...
const ALIASES = {
  ...
  git: {
    ...
    // REBASE
    grb: new AliasDefault('git rebase --ignore-date ${*:::__g-pick-branch --gkcu-returnOutput=1}'),
    grbm: new AliasDefault('git rebase --ignore-date master $*'),
    grbi: new AliasDefault('git rebase --ignore-date --interactive HEAD~$1'),
    grbia: new AliasDefault('git rebase --autosquash --ignore-date --interactive HEAD~$1'),
    grbin: new AliasDefault('git rebase --no-autosquash --ignore-date --interactive HEAD~$1'),
  ...
  docker: {
    ...
    dkratt: new AliasDefault('docker exec -it $1 /bin/bash'),
  ...
  aio: {
    ...
    aiord: new AliasDefault('dkrrd aio aio-builds --dns 127.0.0.1 -p 80:80 -p 443:443 $*'),
  ...
  config: {
    ...
    // GIT
    cfggit: new Alias(new AliasSpec(
      utils.stripIndentation(`
        #!/usr/bin/env node
        'use strict';
        const utils = require('../../lib/utils');
  ...
  misc: {
    // DIRECTORY LISTING
    ll: new AliasDefault('ls --color=auto -hl $*'),
    lla: new AliasDefault('ls --color=auto -ahl $*'),
