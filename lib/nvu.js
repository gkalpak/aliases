...
function _nvu(runtimeArgs, config) {
  ...
  return buildCmd(branch, nvmRuntimeArgs, config).
    then(cmd => {
      if (config.dryrun) {
        console.log(cmd);
...
function buildCmdDefault(branch, runtimeArgs) {
  ...
  if (!hasChainedCmd) {
    ...
    const warning = chalk.yellow(utils.stripIndentation(`
      WARNING:
        Running 'nvm use' like this will not affect the current proccess,
        only the spawned child process. You can still execute a single
        command using the specified Node.js version with '${nvuCmd}'.
...
function buildCmdForWindows(branch, runtimeArgs, config) {
  ...
  const nvlsCmd = utils.getAliasCmd(utils.getAliasSpec(ALIASES.node, 'nvls'));
