...
const PR_REMOTE_ALIAS = 'gcoghpr';
const PR_LOCAL_BRANCH_TOP = 'gcoghpr-top';
const PR_LOCAL_BRANCH_BASE = 'gcoghpr-base';
...
class _Gcoghpr {
    ...
    return Promise.resolve().
      ...
      then(({prId}) => prId && Promise.resolve().
        ...
        then(({branch, originUrl, commits}) => Promise.resolve().
          ...
          then(() => executor.exec(`git remote remove ${PR_REMOTE_ALIAS} || true`)).
          then(() => executor.exec(`git remote add ${PR_REMOTE_ALIAS} ${originUrl}`)).
          then(() => executor.exec(`git fetch --no-tags ${PR_REMOTE_ALIAS} ${branch}`)).
          then(() => executor.exec(`git branch --force --track ${branch} ${PR_REMOTE_ALIAS}/${branch}`)).
          then(() => executor.exec(`git branch --force ${PR_LOCAL_BRANCH_TOP} ${branch}`)).
          then(() => executor.exec(`git branch --force ${PR_LOCAL_BRANCH_BASE} ${branch}~${commits || 0}`)).
          then(() => executor.exec(`git checkout ${branch}`)).
          then(() => this._reportSuccess(branch, commits, executor)).
  ...
  _reportSuccess(branch, commits, executor) {
    let ready = Promise.resolve();
    const commands = [];

    const messageMain = `\nFetched PR into local branch '${chalk.green(branch)}'`;
...
const exps = module.exports = {
  ...
  /** Exposed for testing purposes only. */
  _PR_REMOTE_ALIAS: PR_REMOTE_ALIAS,
