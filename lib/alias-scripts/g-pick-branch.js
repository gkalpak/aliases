...
function pickBranch(branchesStr) {
  ...
  const branches = branchesStr.
    ...
    filter(Boolean).
    map((branch, i) => {
      const [, isCurrentBranch, branchName] = /^(\*\s*)?(\S.*)$/.exec(branch);

      if (isCurrentBranch) currentBranchIdx = i;

      return {
        name: (isCurrentBranch ? `${branchName} (current)` : branchName).replace(/^gcoghpr-.*$/, m => italic(m)),
        value: branchName,
        short: branchName,
      };
    }).
