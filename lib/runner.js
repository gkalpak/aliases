  ...
  let expandedCmd = cmd.replace(re, (_, g1, g2, g3, g4, g5, g6, g7, g8) => {
    ...
    if (!value && g8) {
      ...
      if (!match) {
        ...
        if (!cmdPromises.hasOwnProperty(cmd)) {
          cmdPromises[cmd] = doRun(cmd, runtimeArgs, runConfig).
...
function spawnAsPromised(rawCmd, {debug, dryrun, returnOutput, suppressTbj}) {
  const cleanUp = () => {
    if (debug) {
  ...
  const unsuppressTbj = suppressTbj ? utils.suppressTerminateBatchJobConfirmation(process) : utils.noop;
  const onDone = () => {
    unsuppressTbj();
    cancelCleanUp();
    cleanUp();
