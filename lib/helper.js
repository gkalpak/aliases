...
function helpForCategory(category, joiner) {
  ...
  const headerStyle = category.missing ? chalk.yellowBright : chalk.cyan;
  const aliasStyle = category.missing ? chalk.red : chalk.green;

  return [
    headerStyle(`${utils.capitalize(category.name)} aliases:`),
    '',
    publicAliases.
