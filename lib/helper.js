...
import {capitalize, onError, padRight, wrapLine} from './utils.js';
...
function getDescription(alias) {
  ...
  Object.keys(descReplacements).forEach(original => {
    const repl = descReplacements[original];
...
async function help(...names) {
  ...
  const showCategory = (names.length === 1) && ALIASES.hasOwnProperty(names[0]);
  ...
  try {
    ...
    const matchedCategories = showAliases ?
      ...
      Object.keys(ALIASES).
        ...
        map(categoryName => ({name: categoryName, spec: ALIASES[categoryName]}));
