    ...
    then(() => {
      ...
      const message = [
        ...
        Object.keys(constants.ALIASES).
          ...
          map(name => helpForCategory(name, constants.ALIASES[name], joiner)).
          join(`\n${sep}\n\n`),
        sep,
        '',
...
function helpForCategory(catName, catSpec, joiner) {
  ...
  return [
    ...
    '',
    Object.keys(catSpec).
      map(name => ({name, spec: utils.getSpec(catSpec, name)})).
      map(({name, spec}) => [utils.padRight(name, maxNameLen), utils.wrapLine(spec.desc || spec, indent2)]).
